...
import { AppScreen, CapacityKind, Commitment, DailyCheckIn, MainTab, QuickAddAction, RoutineEntry, weeklyHours } from "@/models/margin";
import { Material, Module, RecoveryResult, dateKey, loadFor, loadPercent, prettyDate, routinePlan, scheduledCommitments, shiftDate, weekStart } from "@/models/planner";
...
export function MarginApp() {
  ...
  else if (screen === "recovery") page = <CurrentRecoveryScreen days={recoveryDays} results={state.recoveryResults} onResult={(key, value) => patch({ recoveryResults: { ...state.recoveryResults, [key]: value } })} onPlan={date => { setSelectedDate(date); showPlan("All"); }} onTab={selectTab} onQuickAdd={quickAdd} />;
  else if (screen === "profile") page = <ProfileScreen streak={streak} commitments={state.commitments} modules={state.modules} routines={state.routineEntries} answers={state.feelAnswers} onTab={selectTab} onQuickAdd={quickAdd} onBaseline={() => { setEditingBaseline(true); setBaselineBack("profile"); setFeelIndex(0); navigate("routine-checklist"); }} />;
  ...
  return <StreakPetContext.Provider value={{ streak, hidden: screen === "profile", checkIn: screen === "today" && checkInEligible ? { label: currentCheck ? "Update today’s check-in" : "Complete today’s check-in", onPress: () => setCheckInVisible(true) } : undefined }}><View style={{ flex: 1 }}>{storageError ? <InlineNotice title="Saving needs attention" body={storageError} tone="amber" /> : null}{page}{checkInSaved ? <View pointerEvents="none" accessibilityRole="alert" accessibilityLiveRegion="polite" style={{ position: "absolute", top: 18, left: 20, right: 20, zIndex: 100 }}><InlineNotice title="Check-in saved" body="Today’s Plan and capacity are up to date." /></View> : null}</View></StreakPetContext.Provider>;
}
