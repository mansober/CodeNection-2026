...
import { Material, Module, RecoveryResult, dateKey, fromKey, loadFor, loadPercent, prettyDate, routinePlan, shiftDate, weekStart } from "@/models/planner";
...
export function MarginApp() {
  ...
  const makeDay = (date: string) => {
    ...
    const base = [...routine, ...state.commitments.filter(item => item.startDate === date || item.dueDate === date || (item.category === "Assignment" && !!item.startDate && item.startDate <= date && (!item.dueDate || item.dueDate >= date)))];
  ...
  const saveCommitment = (item: Commitment) => setState(current => item.routineId ? { ...current, overrides: { ...current.overrides, [item.id]: item } } : { ...current, overrides: Object.fromEntries(Object.entries(current.overrides).filter(([id]) => id !== item.id)), commitments: [...current.commitments.filter(v => v.id !== item.id), item] });
  const onAction = (item: Commitment, action: string, date: string, duration: number, helper: string) => {
    setState(current => {
      const original = item.routineId ? routinePlan(current.routineEntries, current.modules, item.originalDate ?? item.startDate ?? today, current.checks[item.originalDate ?? item.startDate ?? today]).find(v => v.id === item.id) ?? item : current.commitments.find(v => v.id === item.id) ?? item;
  ...
  else if (screen === "plan") page = <CurrentPlanScreen {...menu} filter={filter} days={planDays} selectedDate={selectedDate} onDate={setSelectedDate} onTab={selectTab} onAdd={() => { setEditing(undefined); navigate("add"); }} onEdit={item => { setEditing(item); navigate("add"); }} onAction={onAction} onImport={() => { setEditing(undefined); navigate("import-timetable"); }} />;
  ...
  else if (screen === "add") page = <CommitmentForm key={editing?.id ?? "new"} eyebrow="Your plan" title={editing ? "Edit Commitment" : "Add Commitment"} body="Choose a date and estimate the energy it needs." initial={editing} defaultDate={selectedDate} submitText={editing ? "Save changes" : "Add & continue"} onBack={goBack} onSubmit={item => { saveCommitment(item); showPlan("All"); }} onAddAnother={saveCommitment} />;
