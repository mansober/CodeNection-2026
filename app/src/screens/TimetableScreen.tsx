...
export function ImportTimetableScreen({ initial = [], onBack, onContinue }: { initial?: Module[]; onBack: () => void; onContinue: (modules: Module[]) => void }) {
  ...
  const pick = async () => {
    try {
      ...
      if (/\.ics$/i.test(asset.name)) {
        ...
        setMessage(`${found.length} module names read. Review their class days and assignments below.`);
      } else setMessage("Image text recognition is not connected yet. Use your timetable to add module names and class days below, or import an ICS calendar.");
  ...
  return <ScrollPage><PageHeader eyebrow="Specific commitments" title="Your modules & assignments" body="Import your timetable, review each module, then tell us which modules have an assignment." onBack={onBack} /><View style={s.content}>
    ...
    {modules.map(m => <Card key={m.id} style={{ gap: 10 }}><Text style={s.title}>{m.name}</Text><Text style={s.bodySmallMuted}>Class days each week</Text><View style={s.wrapRow}>{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, i) => <CheckboxRow key={day} label={day} selected={m.days.includes(i)} onPress={() => update(m.id, { days: m.days.includes(i) ? m.days.filter(d => d !== i) : [...m.days, i] })} />)}</View><AppButton text="Remove module" variant="quiet" onPress={() => setModules(current => current.filter(v => v.id !== m.id))} /></Card>)}
