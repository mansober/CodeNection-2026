...
import type { WhatIfComparison } from "@/services/planningApi";
...
export function WhatIfResult({ draft, capacities, comparison, loading = false, onBack, onConfirm, onDecline, onPlan }: { draft: Commitment; capacities: CapacityValue[]; comparison?: WhatIfComparison; loading?: boolean; onBack: () => void; onConfirm: () => void; onDecline: () => void; onPlan: () => void }) {
  const rows = comparison ?? capacities.map(v => ({ kind: v.kind, before: Math.round(v.used / v.limit * 100), after: Math.round((v.used + draft[v.kind]) / v.limit * 100) }));
  ...
  return <ScrollPage><PageHeader eyebrow="Preview · not added" title={draft.name} body={`${draft.startDate ? prettyDate(draft.startDate) : "Illustrative session today · dates not set"} · ${draft.durationHours} hours`} onBack={onBack} /><View style={s.content}>
    ...
    {loading ? <Card tone="mint" style={{ alignItems: "center", gap: 10 }}><ActivityIndicator /><Text style={s.bodySmallMuted}>Checking this commitment against your backend plan…</Text></Card> : null}
    ...
    <AppButton text={overloaded ? "Add with this load" : "Confirm & add commitment"} disabled={loading} onPress={onConfirm} />
